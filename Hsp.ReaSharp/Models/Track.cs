...
public sealed class Track : ReaperObject
{
  ...
  public IEnumerable<MediaItem> EnumerateMediaItems()
  {
    var itemCount = Reaper.CountTrackMediaItems.Invoke(ReaperHandle);
    return Enumerable.Range(0, itemCount).Select(i => MediaItem.FromByTrackIndex(this, i));
  }
  ...
  public MediaItem CreateEmptyItem(TimeSpan? position = null, TimeSpan? length = null)
  {
    var handle = Reaper.AddMediaItemToTrack.Invoke(ReaperHandle);
    var item = MediaItem.FromHandle(handle);
    item.Start = position ?? TimeSpan.FromSeconds(0);
    item.Length = length ?? TimeSpan.FromSeconds(1);
    return item;
  }

  public MediaItem CreateMidiItem(TimeSpan? position = null, TimeSpan? length = null)
  {
    ...
    var handle = Reaper.CreateNewMIDIItemInProj.Invoke(
      ...
      IntPtr.Zero);
    return MediaItem.FromHandle(handle);
  }

  public List<MediaItem> GetSelectedItems(int? maxCount = null)
  {
