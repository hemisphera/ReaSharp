...
function Get-DocumentationVersion {
  param(
  ...
  try {
    Write-Host "Fetching documentation version from $ApiUrl"
    $response = Invoke-WebRequest -Uri $ApiUrl
    $html = $response.Content
    
    if ($html -match 'Generated by REAPER v([\d.]+)') {
      return $Matches[1]
    }
    ...
    Write-Warning "Failed to fetch documentation version: $_"
    return $null
  }
}
...
function Build-GeneratedContent {
  ...
  if (-not [string]::IsNullOrWhiteSpace($Version)) {
    $properties.Add("  public static string GeneratedForReaperVersion { get; } = ""$Version"";")
  }
  ...
  foreach ($signatureRaw in $Signatures) {
    ...
    $properties.Add("  public static ReaperFunc<${functionName}Delegate> $functionName = new(nameof($functionName));")
  }
...
$reaperVersion = Get-DocumentationVersion -ApiUrl $ApiUrl
$versionInfo = if ($reaperVersion) { " (Generated by REAPER v$reaperVersion)" } else { "" }
...
$generated = Build-GeneratedContent -Signatures $signatures -Version $reaperVersion
...
Write-Host "Generated imports for $($generated.Properties.Count) REAPER API functions in $ReaperFile$versionInfo"